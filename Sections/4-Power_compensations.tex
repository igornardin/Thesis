...
% We simplified Equations \ref{equ:hydrogen_electrolyzer} and \ref{equ:hydrogen_fuel_cell}. For the electrolyzer, we changed Equation \ref{equ:hydrogen_electrolyzer}, considering $HH_{h2} = 39$ and $\eta_{ez} = 0.4$, having:
% \begin{align*}
%     E_{ez} &= \frac{HH_{h_{2}} \times Q_{ez}(t)}{\eta_{ez}} \\
%     E_{ez} &= \frac{39 \times Q_{ez}(t)}{0.4}\\
%     E_{ez} &= 97.5 \times Q_{ez}(t)\\
%     Q_{ez}(t) &= \frac{E_{ez}}{97.5}
% \end{align*}
% For the fuel cell, we changed Equation \ref{equ:hydrogen_fuel_cell}, considering $LH_{h2} = 33.33$ and $\eta_{fc} = 0.4$, having:
% \begin{align*}
%     E_{fc} &= LH_{h_{2}} \times Q_{fc}(t) \times \eta_{fc}\\
%     E_{fc} &= 33.33 \times Q_{fc}(t) \times 0.4\\
%     E_{fc} &= Q_{fc}(t) \times 13.332\\
%     Q_{fc}(t) &= \frac{E_{fc}}{13.332}
% \end{align*}
% We use these simplifications to calculate the LoH of Equation \ref{equ:hydrogen_level} and the values come from previous experiments inside the Datazero project. 
